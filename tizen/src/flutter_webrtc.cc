    ...
    void FlutterWebRTC::HandleMethodCall(
        ...
        if (method_call.method_name().compare("createPeerConnection") == 0) {
            ...
            LOG_ERROR("AddCandidate Arguments\nPeerConnectionId: %s\nCandidate: %s\nsdpMid: %s\nsdpLineIndex: %d",
                      peerConnectionId.c_str(), findString(constraints, "candidate").c_str(),
                      findString(constraints, "sdpMid").c_str(), findInt(constraints, "sdpMLineIndex"));
            if (strlen(findString(constraints, "candidate").c_str()) == 0) {
                LOG_ERROR("Candidate is empty");
            }

            if (pc == nullptr) {
                LOG_ERROR("AddCandidateFailed, peerConnection is null");
            ...
            try{
                rtc_candidate = RTCIceCandidate::Create(
                ...
                LOG_ERROR("CAIU NO CATCH DO CreateIceCandidate MALUCO");
            }
            try {
                AddIceCandidate(rtc_candidate.get(), pc, std::move(result));
            } catch (...) {
                LOG_ERROR("CAIU NO CATCH DO AddIceCandidate MALUCO");
            }
